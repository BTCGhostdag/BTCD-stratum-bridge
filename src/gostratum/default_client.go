...
var walletRegex = regexp.MustCompile("BTCD:[a-z0-9]+")
...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "BTCD:") {
		return CleanWallet("BTCD:" + in)
	}

	// has BTCD: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:67], nil
	}
	return "", errors.New("unable to coerce wallet to valid BTCD address")
}
